...
score: 7.0
evidence: 面向高并发LLM推理的KV缓存驱逐与调度策略，可应用于智能体系统
tldr: 针对LLM推理阶段KV缓存造成的内存瓶颈限制高并发服务能力的问题，本文提出压缩分页注意力方法，将词元级KV缓存驱逐与分页注意力相结合，并设计综合调度策略，支持前缀缓存与异步压缩。基于该方法构建的高并发推理引擎Zipage在大型数学推理任务上验证了效果，为多智能体系统等应用提供了高效的缓存管理手段。
...
motivation: LLM推理阶段KV缓存成为内存瓶颈，现有驱逐方法难以用于工业级高并发场景。
method: 提出Compressed PagedAttention，融合词元级KV缓存驱逐与PagedAttention，并设计调度策略、前缀缓存与异步压缩。
result: 在大型数学推理任务上，Zipage维持了高请求并发能力，显著缓解了KV缓存内存压力。
conclusion: 该方法为高并发LLM推理提供了实用且高效的缓存压缩与调度框架，可支撑多智能体等推理应用。
---
