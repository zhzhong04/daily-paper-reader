...
score: 9.0
evidence: 将KV缓存分解为共享基础组件与适配器组件，实现多LoRA智能体间缓存共享
tldr: 多LLM智能体系统常用多LoRA实现角色专业化，智能体共享预训练骨干并仅通过轻量适配器区分。尽管共享基础权重，每个智能体仍为相同的工具增强长轨迹独立构建并存储自己的KV缓存，造成大量内存和计算开销。本文观察到跨智能体缓存差异主要由适配器输出主导，而共享预训练骨干的激活高度相似，据此提出LRAgent，将缓存分解为预训练模型的共享基础组件，实现KV缓存的跨智能体高效共享，降低存储与计算成本。
source: ICML-2026-Accepted
selection_source: conference_retrieval
motivation: 多LoRA智能体共享骨干但各自独立构建全量KV缓存，导致大量冗余存储与计算开销。
method: 基于跨智能体骨干激活高度相似的观察，将KV缓存分解为共享基础组件和适配器特有部分，实现缓存共享。
result: 显著降低多LoRA智能体的KV缓存内存和计算开销，同时保持各智能体的角色专业化能力。
conclusion: 利用共享骨干的相似性设计KV缓存共享，是提升多LoRA智能体系统效率的有效途径。
---
